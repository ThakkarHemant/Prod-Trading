...
const AdminWatchlist = () => {
  ...
  const fetchInitialMarketData = useCallback(async () => {
    setLoading(true)
    try {
      await Promise.all([fetchLTPData(), fetchOHLCData(), fetchQuoteData()])
    } catch (err) {
      setError("Failed to load initial market data")
      console.error("Initial data fetch error:", err)
    } finally {
  ...
  useEffect(() => {
    ...
    const saveToSupabase = async () => {
      try {
        // Clear ALL watchlist items from the table
        const { error: deleteError } = await supabase.from("watchlist_items").delete().neq("id", 0) // This deletes all rows
        if (deleteError) {
          console.error("Error clearing all items:", deleteError)
          return
        }
        ...
        const { error } = await supabase.from("watchlist_items").insert(itemsToSave)
        if (error) {
          console.error("Insert error details:", error)
          return
        }
      ...
      try {
        ...
        if (apiResults.length > 0) {
          const filteredApiResults = apiResults.filter(
            (instrument) => instrument.exchange === "NSE" || instrument.exchange === "MCX",
          )
    ...
    return (
      <div className="max-w-4xl mx-auto p-4 sm:p-6">
        <div className="bg-white rounded-lg shadow-lg p-6 sm:p-8">
          <h2 className="text-xl sm:text-2xl font-bold text-gray-900 mb-4 sm:mb-6">Zerodha Authentication</h2>
          <div className="space-y-4 sm:space-y-6">
            <div className="bg-blue-50 border-l-4 border-blue-500 p-4 rounded">
              <h3 className="text-base sm:text-lg font-medium text-blue-700">One-Click Login</h3>
              <p className="mt-1 text-sm sm:text-base text-blue-600">
                Click below to securely authenticate with Zerodha.
              </p>
            </div>
            ...
            <div className="flex flex-col items-center py-4">
              <button
                onClick={openLoginPopup}
                disabled={loading}
                className={`px-6 sm:px-8 py-3 rounded-lg font-medium flex items-center text-sm sm:text-base ${
                  loading
                    ? "bg-gray-400 cursor-not-allowed"
                    : "bg-orange-600 text-white hover:bg-orange-700 transition-colors"
                }`}
            ...
            <div className="border-t border-gray-200 pt-4">
              <h3 className="text-base sm:text-lg font-medium text-gray-900 mb-2">Troubleshooting</h3>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <div className="p-4 sm:p-5 bg-gray-50 border-b border-gray-200">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4">
            <div>
              <h1 className="text-lg sm:text-xl font-semibold text-gray-800">Market Watchlist</h1>
              <div className="flex flex-wrap items-center gap-2 mt-2">
                <span className="text-xs px-2 py-1 bg-blue-100 text-blue-800 rounded-full">
                  {watchlist.length} instruments
                </span>
            ...
            <div className="flex flex-wrap gap-2 w-full sm:w-auto">
              <button
                onClick={fetchInitialMarketData}
                disabled={loading || isUpdating}
                className="flex items-center gap-2 px-3 py-2 bg-white border border-gray-300 rounded-lg text-sm font-medium hover:bg-gray-50 transition-colors flex-1 sm:flex-none justify-center"
              >
                <RefreshCw className={`h-4 w-4 ${loading || isUpdating ? "animate-spin" : ""}`} />
              ...
              <button
                onClick={logout}
                className="px-3 py-2 bg-red-50 border border-red-200 text-red-600 rounded-lg text-sm font-medium hover:bg-red-100 transition-colors flex-1 sm:flex-none"
              >
                Logout
              </button>
            </div>
        ...
        <div className="p-4 sm:p-5 border-b border-gray-200">
          <div className="relative">
            ...
            <div className="relative">
              ...
              />
              <div className="absolute inset-y-0 right-0 flex items-center pr-3">
                {isSearching && <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-blue-600"></div>}
              </div>
            </div>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th
                  scope="col"
                  className="px-4 xl:px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider"
                >
                  LTP
                </th>
                <th
                ...
                <th
                  scope="col"
                  className="px-4 xl:px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider"
                >
                  Action
                </th>
              </tr>
                ...
                return (
                  <tr key={instrument} className="hover:bg-gray-50">
                    <td className="px-4 xl:px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        <div>
                          <div className="text-sm font-medium text-gray-900">{getInstrumentName(instrument, data)}</div>
                          <div
                    ...
                    <td className="px-4 xl:px-6 py-4 whitespace-nowrap text-right text-sm text-gray-500">
                      {typeof data.volume === "number" && data.volume > 0 ? data.volume.toLocaleString("en-IN") : "-"}
                    </td>
                    <td className="px-4 xl:px-6 py-4 whitespace-nowrap text-right text-sm text-gray-500">
                      {data.ohlc?.high ? formatPrice(data.ohlc.high) : data.day_high ? formatPrice(data.day_high) : "-"}
                    </td>
                    <td className="px-4 xl:px-6 py-4 whitespace-nowrap text-right text-sm text-gray-500">
                      {data.ohlc?.low ? formatPrice(data.ohlc.low) : data.day_low ? formatPrice(data.day_low) : "-"}
                    </td>
                    <td className="px-4 xl:px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                      <button
                        onClick={() => removeFromWatchlist(instrument)}
                        className="text-red-600 hover:text-red-900 p-1 rounded-full hover:bg-red-50"
                        title="Remove from watchlist"
          ...
          {watchlist.length === 0 ? (
            ...
            <div className="divide-y divide-gray-200">
              {watchlist.map((instrument) => {
                ...
                return (
                  <div key={instrument} className="p-4 space-y-3">
                    {/* Header Row */}
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      {hasData ? formatChange(data) : <span className="text-gray-400 text-xs">N/A</span>}
                    </div>
        ...
        <div className="p-3 sm:p-4 bg-gray-50 border-t border-gray-200">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 text-xs text-gray-500">
            ...
            <div>{user && <span>Logged in as {user.user_name}</span>}</div>
          </div>
        </div>
      </div>
    </div>
