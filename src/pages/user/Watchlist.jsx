...
const fetchUserBalance = async () => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      {/* Modal Content */}
      <div className="relative bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-md p-4 sm:p-6 max-h-[90vh] overflow-y-auto">
        <h3 className="font-bold text-lg mb-4">
          Trade {stock?.symbol || 'Stock'}
        </h3>
        
        {/* Stock Info */}
        <div className="bg-base-200 p-3 sm:p-4 rounded-lg mb-4">
          <div className="flex justify-between items-center mb-2">
        ...
        <div className="flex flex-col sm:flex-row gap-3 pt-4">
          ...
          <button
            className={`btn flex-1 order-1 sm:order-2 ${tradeData.action === 'buy' ? 'btn-success' : 'btn-error'}`}
            onClick={handleTradeSubmit}
            disabled={tradeData.loading || (tradeData.action === 'buy' && !canAffordTrade()) || tradeData.quantity < 1}
          >
  ...
  return (
    <div className="space-y-4 sm:space-y-6 p-4 sm:p-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold">Live Watchlist</h1>
                ...
                <tbody>
                  {filteredStocks.map((stock, index) => (
                    <tr key={stock.instrument_key || index}>
                      ...
                      <td className="text-right">🪙{stock.ltp.toFixed(2)}</td>
                      ...
                      <td className="text-right">🪙{stock.high.toFixed(2)}</td>
                      <td className="text-right">🪙{stock.low.toFixed(2)}</td>
                ...
                {filteredStocks.map((stock, index) => (
                  <div key={stock.instrument_key || index} className="border-b border-gray-200 p-4">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-sm mb-3">
                      <div>
                        <span className="text-gray-500">High:</span> 🪙{stock.high.toFixed(2)}
                      </div>
                      <div>
                        <span className="text-gray-500">Low:</span> 🪙{stock.low.toFixed(2)}
                      </div>
