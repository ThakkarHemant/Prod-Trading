...
function UserPortfolio() {
  ...
  useEffect(() => {
    ...
    socket.on("connect_error", (err) => {
      ...
      // Clear real-time data on connection error
      setRealTimeData({});
    });
    ...
    socket.on("disconnect", (reason) => {
      ...
      // Clear real-time data on disconnect to force fallback to cache
      setRealTimeData({});
    });
  ...
  const getCurrentPrice = (instrumentKey, fallbackPrice) => {
    ...
    // Fallback to cached price from portfolio data
    console.log(`Using fallback price for ${instrumentKey}: ${fallbackPrice}`)
    return fallbackPrice || 0
  }

  // Helper function to calculate real-time P&L
  const calculateRealTimePnL = (position) => {
    const currentPrice = getCurrentPrice(position.instrument_key, position.current_price)
    const investmentValue = parseFloat(position.average_price) * parseInt(position.quantity)
    const currentValue = currentPrice * parseInt(position.quantity)
  ...
  const calculateSummary = () => {
    ...
    currentHoldings.forEach(position => {
      ...
      console.log(`${position.instrument_key}: qty=${position.quantity}, avgPrice=${position.average_price}, investment=${investmentValue}, currentValue=${currentValue}`)
          ...
          <div className="card-content">
            <div className="space-y-8">
              <Tabs defaultValue="all" className="w-full">
                <TabsList>
                  <TabsTrigger value="all">All Holdings ({currentHoldings.length})</TabsTrigger>
                  <TabsTrigger value="nse">NSE ({nseStocks.length})</TabsTrigger>
                  <TabsTrigger value="mcx">MCX ({mcxStocks.length})</TabsTrigger>
                </TabsList>
